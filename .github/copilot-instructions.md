...
- Polls the AlphaVantage API on a scheduled basis (every 15 minutes by default)
- Fetches intraday stock prices for user-configured stock symbols (comma-separated list via `ownStocks` config)
- Creates ioBroker states for stock data (open, high, low, close prices and volume) organized by stock symbol
- Requires an AlphaVantage API key configured via `apiKey` native config
- Operates in schedule mode (`*/15 * * * *`) and terminates after each data collection run
...
- Use scheduled mode (`*/15 * * * *`) for periodic stock price updates via AlphaVantage API
- Implement proper API rate limiting and error handling for the AlphaVantage TIME_SERIES_INTRADAY endpoint
- Structure states hierarchically by stock symbol (e.g., `AAPL.1open`, `AAPL.2high`, `AAPL.3low`, `AAPL.4close`, `AAPL.5volume`)
- Validate stock symbols and API key before making requests; terminate with clear error messages if invalid
- Handle market hours and weekend scenarios gracefully (AlphaVantage may return empty or stale data)
- Configuration: `apiKey` (AlphaVantage API key, required), `ownStocks` (comma-separated list of stock symbols)
- Use proper number formatting for financial data (prices as strings from API, volume as integer)
- The adapter terminates after each scheduled run — avoid persistent timers or intervals